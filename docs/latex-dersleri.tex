...
LaTeX (La(mport)TeX), \href{http://www.lamport.org/}{Leslie Lamport}\index{Lamport, Leslie} tarafından 1978'de yazılmaya başlanmış olup TeX üzerine kurulmuştur. Amacı TeX'i daha işlevli ve kolay hale getirmektir.

İki program arasındaki ilişki şöyle ifade edilebilir \citep{Pantieri2017}: \emph{TeX bir vücutsa LaTeX'de onun üzerine oturtulan bir elbisedir}. Günümüzde Frank Mittelbach\index{Mittelbach, Frank} liderliğinde bir grup programcı tarafından güncelleştirilmektedir.
...
LaTeX'de girdi ve çıktı ekranı farklıdır ve çıktıyı görmek için girdinin derleme işleminden geçmesi gerekir. LaTeX'in zor gibi görünmesinin bir nedeni de komutlarla çalışmasıdır. Gerçekten de WYSIWYG programlarına alışkın bir kullanıcı için bu süreç başlangıçta biraz sıkıcı ve zor olabilir. Ancak belli bir aşamaya geldiğinizde komutlarla çalışmanın aslında işleri kolaylaştırdığını göreceksinizdir. Örneğin LaTeX'le yüzlerce hatta binlerce sayfadan oluşan bir kitabın İçindekiler tablosunu oluşturmak için sadece \texttt{\textbackslash{}tableofcontents} komutunu girersiniz ve LaTeX bu komutu girdiğiniz yere İçindekiler tablosunu hatasız bir şekilde oluşturur. Ayrıca örnekte görüldüğü gibi LaTeX komutları doğal dile\footnote{Tabii ki İngilizce.} oldukça yakındır. ``İçindekiler Tablosu'`nun İngilizcesi ``Table of Contents'' olup komut sadece harflerin küçük halde yan yana dizilmesiyle oluşur. Bu da işleri oldukça kolaylaştırır.
...
\section{Neden LaTeX}\label{neden-latex}}
...
\item
  Matematiksel formüller kullanacaksanız, bu LaTeX'in en güçlü olduğu konudur. Formülleri belgenize adeta \emph{inci dizer gibi} dizer.
\item
  Çok yaygındır.\footnote{Maalesef ülkemizde değil.} Uluslararası bir çok yayınevi (örneğin Springer) ve dergi yazarlardan LaTeX dosyalarını talep eder.
\item
